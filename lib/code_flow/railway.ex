...
defmodule CodeFlow.Railway do
  ...
  def validate_is_active(%User{active: true} = user), do: {:ok, user}
  def validate_is_active(_user), do: {:error, "Not an active User"}

  def validate_at_least_age({:ok, %User{age: age} = user}, cutoff_age) when age >= cutoff_age,
    do: {:ok, user}

  def validate_at_least_age({:ok, _user}, _cutoff_age),
    do: {:error, "User age is below the cutoff"}

  def validate_at_least_age(error, _cutoff_age), do: error

  def check_name_blacklist({:ok, %User{name: name} = user}) do
    case name in ["Tom", "Tim", "Tammy"] do
  ...
  def increment_points({:ok, %User{points: points} = user}, inc_point_value),
    do: {:ok, %User{user | points: points + inc_point_value}}

  def(increment_points(error, _inc_point_value), do: error)
end
